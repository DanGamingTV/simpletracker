...
function site_header() {
    ...
    if (array_key_exists('user', $_SESSION)) {
        ...
        printf('<li class="right"><a href="logout.php">Logout</a></li>');
        printf('<li class="right"><a href="%s" title="View your profile">Logged in as %s</a></li>', html_escape(site_url() . "/profile.php") , html_escape($_SESSION['user']['username']));
...
function site_footer() {
    global $CONFIG;
    printf('<footer><p>Any content uploaded has no association or affiliation with %s. %s does not store the files of any torrents, only the torrent files themselves.</p></footer>', html_escape($CONFIG['site_title']), html_escape($CONFIG['site_title']));
    printf('</body>');
}
