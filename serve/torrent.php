...
if (array_key_exists('id', $_GET)) {
    $id = $_GET['id'];
    $res = $db->query_params('SELECT torrent_id, torrents.user_id, anonymous, name, description, data, submitted, info_hash, total_size, username FROM torrents JOIN users on users.user_id = torrents.user_id WHERE torrent_id = :torrent_id', array('torrent_id' => $id)) or die('db error');
    $row = $res->fetch();
}
...
if ($row) {
    ...
    printf('<tr><th>Added</th><td><tt title="%s">%s</tt></td></tr>', html_escape($db->get_datetime($row['submitted'])->format('c')), html_escape($db->get_datetime($row['submitted'])->format('d/m/Y g:i a')));

    if (!$row['anonymous']) {
        printf('<tr><th>Author</th><td><tt><a href="%s">%s</a></tt></td></tr>', html_escape(site_url() . "/user.php?id=" . $row['user_id']), html_escape($row['username']));
    }
